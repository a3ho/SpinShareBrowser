...
The full catalog and its attempt/fetch timestamps are saved in `charts-cache.json`, normally under `%LOCALAPPDATA%\SpinShareBrowser`, independently of the program and chart installation folders. A normal quit keeps them. Full requests to SpinShare remain at least **10 minutes** apart, including failures and app restarts; this runs behind the scenes without a countdown or disabling **Refresh list** for the interval. Updates happen when filtering or refreshing requires them, not through a new background polling timer. The service saves the attempt before requesting data and writes a new catalog atomically before making it available.

If an update fails, times out, or cannot be saved, an available old catalog remains usable with a notice. Without a usable saved catalog, the app reports the real loading or storage error instead of showing an invented empty list. A new catalog invalidates related derived caches while preserving search choices. Automatic catalog queries still use the search API, not the view-counting song detail endpoint; manually opening a website link follows the site's counting rules.
