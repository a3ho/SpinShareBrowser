...
Pages contain **10, 20, or 30** charts. **Unlimited** adds more as you scroll. You can jump to a page directly. **Filter charts** and **Refresh list** automatically reuse the saved catalog or update it when due. After reopening the app, your first filter restores it through the local service; there is no catalog countdown to wait for. Text, tags, sorting, and paging use the loaded list without another full catalog request. A catalog update does not clear those search choices.
...
The full catalog and its attempt/fetch timestamps are saved in `charts-cache.json`, normally under `%LOCALAPPDATA%\SpinShareBrowser`, independently of the program and chart installation folders. A normal quit keeps them. Before sending a catalog request, the service atomically reserves its **10-minute** cooldown; it will not contact SpinShare if that reservation cannot be saved. A confirmed 401/403 access rejection rolls the reservation back. Rate limits, server failures, response waits, interrupted requests, and timed-out, oversized, invalid, or locally uncommitted full responses keep the cooldown across app restarts. Only failures confirmed before the request can reach SpinShare—DNS, TCP/TLS connection setup—or a safely rolled-back access rejection remain immediately retryable. This runs behind the scenes without a countdown or disabling **Refresh list** for the interval; updates happen only when filtering or refreshing requires them.

If an update fails, times out, or cannot be saved, an available old catalog remains usable with a notice. Without a usable saved catalog, the centered error distinguishes network, access, rate-limit, server, transfer, response, and local-storage failures and offers **Retry**. Retry always passes through the local service: only a failure proven to have used almost no server resources may contact SpinShare again immediately; any request that was sent or may have reached the server returns the persisted cooldown until it expires. A new catalog invalidates related derived caches while preserving search choices. Automatic catalog queries still use the search API, not the view-counting song detail endpoint; manually opening a website link follows the site's counting rules.

<!-- Superseded catalog UI: Refresh data showed a ten-minute countdown and was disabled during that interval. Refresh list now shares automatic cache reuse/update with filtering; the backend interval remains. The separate 60-second manual review cooldown is unchanged. -->
