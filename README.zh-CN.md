...
每页可选 **10 条、20 条或 30 条**，也可跳转到指定页码；选择“不限制”后，向下滚动会分批显示更多谱面。“筛选谱面”和“刷新列表”会自动复用已保存目录，到期时再更新。关闭重开后，首次筛选通过本地服务恢复目录，无需等待目录倒计时。文字、标签、排序和翻页使用已加载列表，不会另发全量目录请求；目录更新本身不清空这些搜索选择。
...
鼠标悬停或键盘聚焦谱面封面时会显示播放按钮，点击后由顶栏唯一的全局播放器试听该谱面。歌名保留为可选择、复制的普通文本，旁边独立、低调的外链图标用于打开官网谱面页。顶栏播放器显示较大封面、歌名与艺人、当前和总时间及可拖动进度条，不显示难度标签。已有曲目时，Space 可全局切换播放 / 暂停；正在编辑或使用原生以 Space 操作的控件时让出快捷键。没有当前曲目时，Space 保持原有行为，不会根据悬停谱面自动开始播放。

每次试听最多 25 秒。程序只把严格校验后的 `fileReference` 拼接到固定的 SpinShare CDN 音频路径，不访问曲目详情、下载或其它计数接口。播放器先尝试 OGG，失败后只回退一次 MP3，每种格式的加载上限为 15 秒。筛选、标签、排序和翻页会保留当前曲目；目录更新发现媒体身份变化时才清理。隐藏或最小化程序会暂停，恢复可见后不自动续播；完全退出时释放媒体。曲目切换使用统一短动效和双封面交叉淡化，减少运动偏好会取消空间位移和缩放。

作者说明不参与搜索，也不再在卡片内滚动。短说明完整显示，原生链接可用，没有光晕或展开操作；长预览完整露出实际文字的半行，再用额外 3px 做末端渐隐，避免把半行主体遮淡得难以辨认。悬停、键盘聚焦或打开期间，文字与实体预览表面抬升 2px，背后呈现扩散柔光，外部命中区域保持不动。普通悬停没有亮色轮廓，键盘保留边缘焦点提示；减少运动时取消位移。绝不悬停自动打开，点击、Enter 或 Space 才会打开完整说明；长预览内部为 inert，选择、复制和点击链接均在全文浮层中进行。

描述浮层只显示全文正文，不添加可见歌名或标题，曲目上下文保留在可访问名称中；评论浮层标题不变。它使用已加载的内容与安全链接，不新增服务器请求，并与临时评论及标签浮层协调。浮层内任意位置的普通滚轮都只滚内部，包括空白、边缘、短内容和滚动边界，Ctrl+滚轮保留原有行为。它与评论浮层统一采用 220ms 进入、150ms 退出的节奏，阅读期间停稳，遵循减少运动偏好，不设 X，也不改变页面高度。离开来源卡片与浮层后保留 160ms 容错再关闭，Escape 或外部点击也可关闭。页面滚动只在首次事件启动同一退场，连续滚轮或惯性滚动不会反复重启动画、延迟隐藏。标签独立留在底部一行，不触发说明展开；悬停、键盘聚焦或触摸仍可查看全部标签。
...
全量目录及尝试、获取时间保存在 `charts-cache.json`，默认位于 `%LOCALAPPDATA%\SpinShareBrowser`，与程序安装目录和谱面目录独立，正常完全退出后保留。两次从 SpinShare 全量获取数据仍至少间隔 **10 分钟**，失败和重启也不能绕过；这一过程在后台处理，不显示目录倒计时，也不因间隔未到而禁用“刷新列表”。筛选或刷新时才按需要更新，不新增后台定时联网。服务会在请求前保存尝试时间，新目录先原子写入本地文件，再提供给界面。

更新失败、超时或无法保存新数据时，若仍有可用旧目录，就提示正在使用上次保存的数据并继续筛选；首次没有可用缓存时显示真实加载或存储错误，不能假装得到空列表。新目录会使相关派生缓存失效，但保留搜索选择。自动目录查询仍使用搜索接口，不访问会增加浏览量的曲目详情页；手动打开官网链接仍遵循官网的计数规则。

<!-- Superseded：先前“更新数据”显示十分钟倒计时并在冷却时禁用；当前“刷新列表”与筛选共用缓存复用及到期更新，后台间隔仍保留。评论手动刷新的 60 秒倒计时不受影响。 -->
