...
再次运行安装包会明确以覆盖更新方式替换已安装的 SpinShare Browser，保留设置，也不会修改游戏 `Custom` 目录中已经安装的谱面。通过 Windows“设置 → 应用”或控制面板“程序和功能”卸载。卸载会删除工具及其设置、缓存等本地数据，保留下载或安装的谱面以及共享 WebView2 运行时。
